...
SELECT DISTINCT
	...
	LOGEMENT.CODE_TYPE_LOGEMENT,
	TYPE_LOGEMENT.DESCRIPTION,
	SUM(LEAST(RESERVATION.FIN_SEJOUR, TO_DATE('23-03-2015', 'dd-mm-yyyy')) - GREATEST(RESERVATION.DEBUT_SEJOUR, TO_DATE('07-03-2015', 'dd-mm-yyyy'))) / 16 * 100 || '%' AS TAUX_OCCUPATION
FROM
	LOGEMENT
		INNER JOIN TYPE_LOGEMENT
			ON LOGEMENT.CODE_TYPE_LOGEMENT = TYPE_LOGEMENT.CODE_TYPE_LOGEMENT
		LEFT JOIN SEJOUR
			ON LOGEMENT.NO_LOGEMENT = SEJOUR.NO_LOGEMENT AND
			   LOGEMENT.NOM_VILLAGE = SEJOUR.NOM_VILLAGE
				INNER JOIN RESERVATION
					ON SEJOUR.NO_RESERVATION = RESERVATION.NO_RESERVATION AND
					   SEJOUR.NOM_VILLAGE = RESERVATION.NOM_VILLAGE AND 
					   LOGEMENT.NOM_VILLAGE = 'Casa-Dali' 
HAVING
	SUM(LEAST(RESERVATION.FIN_SEJOUR, TO_DATE('23-03-2015', 'dd-mm-yyyy')) - GREATEST(RESERVATION.DEBUT_SEJOUR, TO_DATE('07-03-2015', 'dd-mm-yyyy'))) / 16 * 100 < 30 AND
	...
	OR
	 (RESERVATION.FIN_SEJOUR > TO_DATE('23-03-2015', 'dd-mm-yyyy') AND
	  RESERVATION.DEBUT_SEJOUR < TO_DATE('07-03-2015', 'dd-mm-yyyy')))
GROUP BY
	LOGEMENT.NO_LOGEMENT,
